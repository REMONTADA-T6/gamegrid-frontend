...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const res = await axios.get("https://gamegrid-backend.onrender.com/api/scans/leaderboard");
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 bg-white rounded-lg shadow-sm">
          <thead className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white ">
            <tr>
              <th className="px-6 py-3 text-left text-xl font-bold">Rank</th>
              <th className="px-6 py-3 text-left text-xl font-bold">Player</th>
              <th className="px-6 py-3 text-left text-xl font-bold">Total Points</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 text-gray-900 text-center ">
            {leaderboard.map((team, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-3 font-medium">{team.team}</td>
